...
having 
  count(customer_id) > 300
-- исправлена группировка по уникальным значениям
...
from 
  inventory i 
  join rental r using (inventory_id)-- уникальное 
  join film f using (film_id) -- уникальное
  join language l using(language_id) --уникальное
  join film_category fc using (film_id) --уникальное
  join category c using (category_id) --уникальное
  join payment p using (rental_id)-- уникальное
group by 
  film_id, 
  l.language_id,  
  c.category_id 
...
from 
  film f 
  left join inventory i using(film_id)--уникальное 
  left join rental r using(inventory_id)--уникальное
  left join payment p using(rental_id)--уникальное
  join language l using(language_id)--уникальное 
  join film_category fc using(film_id)--уникальное
  join category c using(category_id) -- уникальное
where  
...
group by 
  p.staff_id 
-- исправлено всего 2 сотрудника и они взяты из таблицы платежи т.е. продажи
